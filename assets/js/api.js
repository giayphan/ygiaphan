...
(function(){
  ...
  window.YP_API = {
    ...
    vocabSave(vi,th,slug,ph){ return postJSON({action:'vocab_save', token: window.YP_AUTH.token(), vi, th, slug, ph: ph||''}); },
    vocabDue(){ return postJSON({action:'vocab_due', token: window.YP_AUTH.token()}); },
    vocabList(){ return postJSON({action:'vocab_list', token: window.YP_AUTH.token()}); },
    vocabDelete(vi){ return postJSON({action:'vocab_delete', token: window.YP_AUTH.token(), vi}); },
